"""
run_pipeline_hands.py

Hand-movement based active speaker detection + AMI ground-truth scoring,
in one run. Mirrors run_pipeline.py / run_pipeline_lips.py but uses average
per-landmark displacement of detected hand landmarks as the movement signal.

Just run:
    python run_pipeline_hands.py --meetings-xml "C:\\path\\to\\meetings.xml"

Fixes applied vs. your original hand detection script:
  1. Same fps-consistency check across all 4 videos.
  2. Same "Silence" decision instead of always crowning a winner when all
     scores are 0/near-0.
  3. Output CSV now stores WindowStartSec/WindowEndSec (seconds) instead of
     only a raw Frame number.
  4. Added --headless mode, file-existence checks, auto-discovery of
     meetings.xml + segments/words xml (dotted or underscored naming).
  5. Runs the AMI evaluation immediately after detection, in the same run.
  6. IMPORTANT CAVEAT (not silently "fixed", flagged instead -- see note
     below): landmark index-matching between frames is unreliable for
     hands specifically, more so than for the single-face head/lip cases.

Same Person->Closeup->AMI-channel caveat as before, resolved automatically
from meetings.xml.

----------------------------------------------------------------------
KNOWN LIMITATION -- landmark correspondence between frames (hands only)
----------------------------------------------------------------------
Your original movement calculation pairs current_points[i] with
prev[i] by raw list position. For a single face (head/lips scripts) this
is safe: there's always exactly one face, so index i always means the
same landmark. For hands this is NOT safe:
  - MediaPipe's hand_landmarks list order can flip between frames purely
    because it's ordered by internal detection confidence, not by a
    stable "left hand first" rule -- so index i can silently jump from
    meaning "left thumb tip" to "right thumb tip" between two
    consecutive frames.
  - The number of visible hands can also change frame to frame (0, 1, or
    2 hands), which shifts every subsequent index.
Both effects can inject large spurious "movement" that has nothing to do
with the person actually gesturing. I have NOT silently patched this with
a heuristic, because a wrong guess here (e.g. matching by nearest
centroid) would quietly change your results without you deciding that
tradeoff. If your hand-detection accuracy looks a lot noisier than the
head/lip versions, this is the most likely reason, and the fix would be
to match hands by handedness label (result.handedness) and landmark
identity rather than list position -- happy to implement that with you
if the raw scores look unusable, but wanted to flag it first rather than
change your algorithm's behavior for you.
"""
...
DEFAULT_MODEL_PATH = r"C:\Users\Ehtiram\Downloads\hand_landmarker.task"
...
def run_detection(args):
    ...
    previous_landmarks = {person: None for person in video_paths}
    ...
    try:
        while current_frame <= end_frame:
            ...
            for person, cap in caps.items():
                ...
                # NOTE: see module docstring "KNOWN LIMITATION" -- this still
                # pairs points by raw list position, same as your original
                # script. Left unchanged pending your call on whether to
                # switch to handedness-based matching.
                if previous_landmarks[person] is not None:
                    prev = previous_landmarks[person]
                    n = min(len(prev), len(current_points))
                    if n > 0:
                        total = 0
                        for i in range(n):
                            total += np.linalg.norm(
                                np.array(current_points[i]) - np.array(prev[i])
                            )
                        movement = total / n

                previous_landmarks[person] = current_points
...
def parse_args():
    p = argparse.ArgumentParser(description="Hand-movement speaker detection + AMI evaluation, one shot")
    ...
    p.add_argument("--movement-threshold", type=float, default=1.0,
                    help="Tune based on your printed Window Result values -- hand movement scores "
                         "are in raw pixel units per landmark, similar order of magnitude to the "
                         "head script.")
    p.add_argument("--silence-threshold", type=float, default=0.1)
    p.add_argument("--headless", action="store_true", default=False)
    p.add_argument("--output", default="HandSpeakerScores.csv")
    p.add_argument("--output-dir", default="eval_out_hands")
    return p.parse_args()
...
def main():
    ...
    print("=== STEP 1: Running hand-movement detection ===")
