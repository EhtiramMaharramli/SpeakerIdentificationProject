"""
run_pipeline_lips.py

Lip-movement based active speaker detection + AMI ground-truth scoring,
in one run. Mirrors run_pipeline.py (head movement) but uses mouth-opening
delta instead of face-bbox-center displacement as the movement signal.

Just run:
    python run_pipeline_lips.py

Fixes applied vs. your original lip detection script:
  1. Same fps-consistency check across all 4 videos (was silently trusting
     Person1 only).
  2. Same "Silence" decision: if nobody's mouth moves meaningfully in a
     window, label it "Silence" instead of defaulting to whichever Person
     happens to be first in the dict when all scores are 0.
  3. previous_opening is now reset to None when a face isn't detected in a
     frame, so a stale opening value from before a detection gap can't
     produce a fake "jump" score once the face reappears (same issue the
     head-movement script had with previous_centers).
  4. Output CSV now stores WindowStartSec/WindowEndSec (seconds) instead of
     only a raw Frame number, so it aligns to the AMI transcript without
     needing to separately track/guess fps.
  5. Added --headless mode, existence checks for model/video files, and
     auto-discovery of meetings.xml + segments/words xml (same as
     run_pipeline.py), supporting both dotted (ES2008a.A.words.xml) and
     underscored naming.
  6. Runs the AMI evaluation immediately after detection, in the same run.

Same Person->Closeup->AMI-channel caveat as before: PersonN = CloseupN, but
CloseupN is NOT always AMI channel N-th letter. This is resolved
automatically from meetings.xml, same as run_pipeline.py.
"""
...
DEFAULT_MODEL_PATH = r"C:\Users\Ehtiram\Downloads\face_landmarker.task"
...
# ----------------------------------------------------------------------
# File discovery helpers (same as run_pipeline.py)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# AMI ground truth parsing (identical to run_pipeline.py)
# ----------------------------------------------------------------------
...
def run_detection(args):
    ...
    previous_opening = {person: None for person in video_paths}
        ...
        while current_frame <= end_frame:
            ...
            for person, cap in caps.items():
                ...
                if result.face_landmarks:
                    ...
                    upper, lower = face[13], face[14]
                    left, right = face[78], face[308]
                    ...
                    mouth_open = np.linalg.norm(upper_pt - lower_pt)
                    mouth_width = np.linalg.norm(left_pt - right_pt)

                    if mouth_width > 0:
                        opening = mouth_open / mouth_width
                        if previous_opening[person] is not None:
                            score = abs(opening - previous_opening[person])
                        previous_opening[person] = opening
                else:
                    # FIX: no face this frame -> don't let a stale previous_opening
                    # produce a fake jump once the face reappears.
                    previous_opening[person] = None

                window_scores[person] += score
...
# ----------------------------------------------------------------------
# Evaluation (identical logic to run_pipeline.py)
# ----------------------------------------------------------------------
...
def parse_args():
    p = argparse.ArgumentParser(description="Lip-movement speaker detection + AMI evaluation, one shot")
    ...
    p.add_argument("--movement-threshold", type=float, default=0.5,
                    help="Mouth-opening-ratio delta sum below which a window counts as Silence. "
                         "Lip movement scores are much smaller in magnitude than head-movement pixel "
                         "scores, so this default is much lower than the head script's -- tune based "
                         "on your printed Window Result values.")
    p.add_argument("--silence-threshold", type=float, default=0.1)
    p.add_argument("--headless", action="store_true", default=False)
    p.add_argument("--output", default="LipSpeakerScores.csv")
    p.add_argument("--output-dir", default="eval_out_lips")
    return p.parse_args()
...
def main():
    ...
    print("=== STEP 1: Running lip-movement detection ===")
